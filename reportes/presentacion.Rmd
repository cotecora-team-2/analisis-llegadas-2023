...
* Se utilizan las elecciones nacionales del 2018, pues las elecciones estatales del 2017 fueron organizadas localmente, los resultados pueden ser diferentes y los datos tienen formatos diferentes. Adicionalmente, el caso de Coahuila 2017 es muy extremo en cuánto a censura de llegada de casillas a la muestra como discutimos anteriormente.
...
* Algunas de las covariables utilizadas en el modelo son el tipo de sección (urbana o rural), el tipo de casilla (básica, contigua, especial o extraordinaria), la lista nominal, y el porcentaje de votos obtenidos por los 3 principales candidatos de 2018. 
...
## Archivos de muestras para pruebas
...
1. El formato es el mismo que el de los cómputos distritales 2017 de cada estado. El marco que
se usa es el mismo (quitando aquellas casillas que no tienen distrito en ID_DTTO_LOCAL_Ago2022 o
tienen NA en votos nulos). Las muestras son de 450 casillas para Coahuila y 700 para el estado de México y se utiliza la estratificación dada por ID_DTTO_LOCAL_Ago2022. 
2. Hay dos variables adicionales: num_muestra (1 a 100) y corte (50, 75, 90, 100)
3. Necesitamos estimaciones de intervalos para cada num_muestra y para cada corte.
4. Subiremos a Dropbox las muestras, los marcos y un ejemplo de salida en formato csv.


## Ejemplo

Utilizamos nuestro estimador de razón para evaluar éstas muestras. Estimamos CAND_0,...,CAND,5, OTROS y PART, donde OTROS es la suma de NUM_VOTOS_CAN_NREG y NUM_VOTOS_NULOS (podemos hacer caso omiso a las coaliciones).
...
A continuación analizamos las coberturas obtenidas, es decir, el porcentaje de veces en que el intervalo contiene el verdadero valor. 
...
```{r, echo = FALSE, include = FALSE}
coberturas_tbl <- bind_rows(resultados) |>
