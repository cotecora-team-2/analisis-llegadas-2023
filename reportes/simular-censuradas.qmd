...
muestras_lst <- map(1:2, function(i){
  ...
  write_csv(computos |> select(!c(contains("cand_", ignore.case = FALSE), starts_with("total_std"))), 
            paste0("./muestras/marco-", tolower(estado_abbr), "-2017.csv"))
  frac <- frac_muestreo[i]
  p_azar <- prop_azar[i]
  print(estado_abbr)
  muestras_edo_tbl <- map_df(1:150, function(rep){
    ...
    muestras_censuradas_tbl <- map_df(cortes, function(corte_selec){
      ...
      muestra_censurada |> select(-c(cand_1:cand_3, cand_1_std:tiempo))
      })
