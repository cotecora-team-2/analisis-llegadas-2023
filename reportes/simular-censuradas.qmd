...
muestras_lst <- map(1:2, function(i){
  ...
  computos <- computos |> 
    ...
    mutate(TOTAL = CAND_0 + CAND_1 + CAND_2 + CAND_3 + CAND_4 + CAND_5 + OTROS) |>
    filter(LISTA_NOMINAL >= TOTAL) |>
    relocate(c(CAND_0:CAND_5, OTROS, TOTAL, LISTA_NOMINAL), .after = num_casilla)
  write_csv(computos |>
          select(!c(contains("cand_", ignore.case = FALSE), starts_with("total_std"))) ,
      paste0("./muestras/marco-", tolower(estado_abbr), "-2017.csv"))
  frac <- tam_muestra[i]/nrow(computos)
  p_azar <- prop_azar[i]
  print(estado_abbr)
  muestras_edo_tbl <- map_df(1:100, function(rep){
    ...
    muestras_censuradas_tbl <- map_df(cortes, function(corte_selec){
      ...
      muestra_censurada |> 
        ...
        relocate(c(CAND_0:CAND_5, OTROS, TOTAL, LISTA_NOMINAL), 
                 .after = num_muestra)
      })
