...
Se examinó el desempeño de distintos métodos para integrar los tres intervalos de estimación en los estados de Coahuila y Estado de México. Para este análisis se simularon muestras aleatorias usando como marco los cómputos distritales para las elecciones estatales de gobernador del 2017. Las pruebas se realizaron con 100 muestras aleatorias por estado y se consideraron tres escenarios de censura, cuando han llegado 50% de las casillas en muestra, 75% de las casillas y 90% de las casillas.

Las muestras que no han recibido todas las casillas se denominan muestras censuradas. Para llevar a cabo nuestro análisis, fue crucial desarrollar un método para generar muestras censuradas que se asemejen al proceso real de llegada de información. Para ello, ajustamos un modelo de supervivencia para los tiempos de llegada de las casillas en las elecciones federales de 2018, y validamos el ajuste a lo largo de distintos estados que incluyeron Coahuila y Estado de México. Posteriormente utilizamos el modelo para simular muestras con tiempos de llegada y poder así censurarlas.

Dado que los intervalos producidos en el conteo rápido son de al menos 95% de confianza, el objetivo es buscar la mejor forma de integrar las tres estimaciones individuales, verificando que el valor real esté contenido en el intervalo compulsado aproximadamente 95% de las veces. Adicionalmente, se revisó también que el ancho del intervalo compulsado sea razonable.

A continuación aparece una gráfica con el porcentaje promedio de veces que el valor real cae en el intervalo (a este porcentaje se le conoce como cobertura) para los distintos porcentajes de muestra recibida. Tenemos las coberturas para los tres métodos de estimación individuales, y para dos metodologías de integración: 1) la mediana, que consiste en elegir la mediana de los límites inferiores y la mediana de los límites superiores para cada candidato, y 2) la unión de los intervalos, donde se elige el mínimo de los límites inferiores y el máximo de los superiores.
...
resumen_tamano_tbl <- resultados_tbl |> 
  ...
  summarise(`margen de error` = mean(LS - LI) / 2, .groups = "drop") |> 
...
Queremos verificar también el margen de error de los intervalos compulsados, considerando que intervalos más angostos (menor margen de error) son más informativos. A continuación, graficamos el margen de error promedio para los distintos porcentajes de muestra recibida, para los tres métodos de estimación individuales, para la mediana y para la unión.


```{r, echo = FALSE,  fig.width = 8}
ggplot(resumen_tamano_tbl |> filter(corte != 100, candidato != "PART", candidato != "OTROS")) +
  geom_point(aes(x = corte, y = `margen de error`, colour = candidato)) +
  geom_line(aes(x = corte, y = `margen de error`, colour = candidato)) +
  facet_grid(estado ~ nombre, scales = "free_y")  +
  geom_hline(yintercept = 1.5) +
  labs(subtitle = "Media del margen de error") 
```

Observemos que el margen de error de la unión de los intervalos es comparable en cada caso con el intervalo más ancho de los tres métodos individuales. Así pues, se decide utilizar la unión para integrar las estimaciones de votación para cada candidato.
...
Notamos que la mediana basta para tener coberturas mayores al 95\%. Más aún, analizando la siguiente tabla de márgenes de error promedio,

```{r, echo = FALSE,  fig.width = 3}
resumen_tamano_tbl |> filter(corte != 100, candidato=="PART") |>
  select(-candidato) |>
  mutate(`margen de error` = round(`margen de error`, 2)) |>
  pivot_wider(names_from = nombre, values_from = `margen de error`) |> gt()
```
...
En el caso de los votos de los candidatos, se empleará la unión para generar los intervalos compulsados, lo que nos permitirá lograr una cobertura superior al 95\% en ambos estados. Además, la precisión obtenida mediante la unión de los intervalos es comparable a la del intervalo más amplio de los tres métodos individuales.

En cuanto a la participación ciudadana, utilizaremos la mediana para integrar los tres intervalos, lo que nos permitirá obtener coberturas superiores al 95\% e intervalos con márgenes de error razonables.
